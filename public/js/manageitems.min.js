...
{
  newSelectedObject(document.getElementById("selectedId").innerText, id);
}
...
function newSelectedObject(a, b) {
    ...
    var c = SERVER_ABSOLUTE_PATH + "/objets/getTableauRacine";
    $.post(c, {
        object: a,
        user: b,
        nb: "8"
    }, function(c) {
        newupdateTabItems(c, b, a), window.history.pushState("Test", "Objets - Patrimoine", SERVER_ABSOLUTE_PATH + "/objets/show/" + a);
...
function newupdateTabItems(objets, id)
{
  var divObjets = document.getElementById("ObjectTab");
  divObjets.innerHTML = "";

  var table = document.createElement("table");
  showItems(objets, 0, table, id);

  divObjets.appendChild(table);
}

function showItems(_objets, _level, _table, _id)
{
  for (var i = 0; i < _objets.length; i++)
  {
    ...
    if (typeof _objets[i].head == "undefined")
    {
      var td = document.createElement("td");
      var a = document.createElement("a");
      a.setAttribute("href", "#");
      a.setAttribute("onclick", "newSelectedObject(" + _objets[i].ObjectId + ", " + _id + ")");
      a.appendChild(document.createTextNode(_objets[i].ObjectName));
      td.appendChild(a);
      tr.appendChild(td);
    }
    else
    {
      if (_objets[i].head)
      {
        ...
        th.appendChild(document.createTextNode(_objets[i].ObjectName));
        tr.appendChild(th);

        var div = document.createElement("div");
        div.setAttribute("id", "selectedId");
        div.setAttribute("hidden", "true");
        div.appendChild(document.createTextNode(_objets[i].ObjectId));
        tr.appendChild(div);
      }
      else
      {
    ...
    if (typeof _objets[i]["container"] != "undefined")
    {
      if (_objets[i]["container"] != null)
      {
        showItems(_objets[i]["container"], _level + 1, _table, _id);
      }
    }
  }
}
...
function updateTabItems(a, b) {
    ...
    $.each(a, function(a, c)
    {
        $("#ObjectTab > table").append("<tr>"),
        c.head ? (
          $("#ObjectTab > table > tbody > tr:last").append("<th>"),
          c.link ? (
            $("#ObjectTab > table > tbody > tr > th:last").append('<a href="#" id="parent"' + c.id + ' onclick="newSelectedObject(' + c.id + "," + b + ')">' + c.name + '</a><div class="hide" id="parentId">' + c.id + "</div>"),
            $("input[name='objectId']").val(c.id)
          )
          : (
              $("#ObjectTab > table > tbody > tr:last").append('<div id="selectedId" hidden="true">' + c.id + "</div>"),
              $("#ObjectTab > table > tbody > tr > th:last").append(c.name)
          )
        )
          : c.plus ? (
            $("#ObjectTab > table > tbody > tr:last").append("<strong>"), $("#ObjectTab > table > tbody > tr:last > strong:last").append("<td>"),
            $("#ObjectTab > table > tbody > tr:last > strong:last > td:last").append('<a href="#" onclick="showMore(' + $("#selectedId").html() + "," + b + "," + (parseInt(c.nb) + 8) + ')">' + c.name + "</a>")
          )
          : c.container ? (
            $("#ObjectTab > table > tbody > tr:last").append("<td>"), $("#ObjectTab > table > tbody > tr > td:last").append('<a href="#" onclick="newSelectedObject(' + c.id + "," + b + ')">' + c.name + "</a>")
          )
        : (
          $("#ObjectTab > table > tbody > tr:last").append('<em class="small-tab">'),
          $("#ObjectTab > table > tbody > tr:last > em:last").append("<td>"),
          $("#ObjectTab > table > tbody > tr:last > em:last > td:last").append('<a href="#" onclick="newSelectedObject(' + c.id + "," + b + ')">' + c.name + "</a>")
        )
    }), $("#ObjectTab").append("</table>")
}
...
function submitAddItem() {
    ...
    var c = $("#txtAddName").val(),
		...
		z = $("#chkAddRacine").prop("checked");
    if (0 == $("#selectedId").length || z) var k = null;
    ...
    $.post(target, {
        ...
        owner: l,
        quantity: j
    }), $("#addModal").foundation("reveal", "close"), clearAddModal(), 0 == $("#ObjectImage").length && location.reload(), location.reload(), newSelectedObject(k, l)
}
    ...
    var v = SERVER_ABSOLUTE_PATH + "/objets/getEncodedImagesById";
        ...
        success: function(responseText) {
			...
			if(b != null)
			{
				...
				alert("Coucou");
			//	loadImages(a);
			//	window.location.reload("objets/show/" + a);
		}
    });
...
function generateReport() {
    var a = $("#selectedId").html(),
        b = SERVER_ABSOLUTE_PATH + "/objets/generateReportAll";
    $.post(b, {
        object: a
    }, function(a) {
        $("body").html(a)
